import { contentRows, eventDictionary, platforms, searchRows } from "../data/mockData";
...
export const METRIC_META: Record<MetricId, MetricMeta> = {
  ...
  pageViews: { name: "页面曝光", unit: "number", aggregation: "sum", positive: true },
  pageClicks: { name: "页面点击", unit: "number", aggregation: "sum", positive: true },
  currentPaidMembers: { name: "当前付费会员", unit: "number", aggregation: "latest", positive: true },
  historicalPaidMembers: { name: "历史付费会员", unit: "number", aggregation: "latest", positive: true },
  renewalRate: { name: "续费率", unit: "percent", aggregation: "weighted", positive: true }
  ,videoWatchCount: { name: "视频播放次数", unit: "number", aggregation: "sum", positive: true }
...
function buildInsights(model: AnalysisModelId, filters: DashboardFilters) {
  ...
  if (model === "payment_conversion" || model === "member_operation") return [`${lowestPay?.platform ?? "TikTok"} 是当前汇总付费率最低的平台。`, "VIP 点击到拉起支付是当前汇总漏斗的主要损耗段。", "接口未提供支付通道和会员档位，暂不输出对应结论。"];
  ...
  return [`${bestRevenue?.platform ?? "Pornhub"} 贡献当前范围最多收入。`, `${lowestPay?.platform ?? "调教师"} 付费率低于总体，建议拆解支付通道与会员档位。`, "整体活跃保持增长，但留存改善慢于收入增长。"];
}
