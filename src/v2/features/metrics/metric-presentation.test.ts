...
test("启动结果分母、有效观看门槛和短视频例外完整保留", () => {
  assert.match(explanation("M080").formula, /已上报最终结果的启动数 × 100%/);
  const effective = explanation("M035").text;
  for (const fact of ["首帧成功", "至少 15 秒", "不足 15 秒", "大于 0", "完整播放", "同一有效观看规则", "去重用户数"]) assert.ok(effective.includes(fact), fact);
  const currentRule = catalog.items.find((item: { id: string }) => item.id === "M034").authority.registeredFormula;
  assert.ok(currentRule.includes("watch_duration_ms>=15000") && currentRule.includes("0<video_duration_ms<15000") && currentRule.includes("watch_end_reason=completed"), "有效观看规则变化时必须更新业务释义");
  assert.match(explanation("M031").formula, /结果等待窗口已结束/);
});
