...
const BUSINESS_TERMS: Record<string, string> = {
  ...
  "task_action_click.action_type=claim_reward": "用户提交任务奖励领取",
  "task_reward_result.result_status=success": "任务奖励最终发放成功",
  "benefit_claim_result.result_status=success": "福利领取最终成功",
  "share_invite_bind_result.result_status=success": "邀请绑定最终成功",
  "share_invite_reward_result.result_status=success": "邀请奖励最终发放成功",
  "recharge_credit_status_change.credit_status=success": "充值已成功到账",
  "author_interaction_result.action_id=follow_author": "关注作者操作生效",
  "video_interaction_result.action_id=like": "视频点赞操作成功生效",
  "video_interaction_result.action_id=favorite": "视频收藏操作成功生效",
  "community_comment_result.result_status=success": "社区评论或回复发布成功",
  "effective_play_flag=true 且 effective_play_rule_version=effective_play_v2": "按当前有效观看规则完成至少一次观看",
  "effective_play_rule_version=effective_play_v2 且 effective_play_flag=true": "按当前有效观看规则完成至少一次观看",
  "session_effective_play_seq>=2": "完成至少两次有效观看",
  "play_trigger_type=user_click": "由用户主动点击触发的播放",
  "effective_play_flag=true": "达到有效观看标准的",
  "video_duration_ms>=15000": "视频时长至少 15 秒",
  "watch_duration_ms>=15000": "实际前台观看时长至少 15 秒",
  "watch_duration_ms>=3000": "实际前台观看时长至少 3 秒",
  "3000<=video_duration_ms<15000": "视频时长达到 3 秒且不足 15 秒",
  "0<video_duration_ms<3000": "视频时长大于 0 且不足 3 秒",
  ...
  "paywall_instance_id": "付费墙展示标识",
  "recharge_request_id": "支付提交标识",
  "recharge_session_id": "充值会话标识",
  "order_id": "支付订单标识",
  "credit_status=success": "充值已成功到账",
  "credit_status": "到账状态",
  "payment_status": "第三方支付状态",
  ...
  "app_launch": "应用启动",
  "launch_id": "启动标识",
  "reward_recipient_user_id": "奖励实际接收用户",
  "reactivation_latency_seconds": "再次激活耗时",
  "page_instance_id": "页面实例标识",
  "page_instance_id + request_id + list_id + post_id + list_position": "页面实例、请求、列表、帖子和列表位置",
  "page_instance_id + upsell_id + cta_id": "页面实例、会员引导和操作入口",
  "entry_id": "入口标识",
  "reward_recipient_type": "奖励接收方类型",
  "benefit_claim_result": "福利领取结果",
  "launch_type": "启动类型",
  ...
  "community_comment_result": "社区评论发布结果",
...
export function metricBusinessExplanation(metric: MetricExplanationSource, names: ReadonlyMap<string, string>) {
  ...
  const effectiveRule = metric.authority.registeredFormula.includes("effective_play_flag=true") && (!metric.authority.registeredFormula.includes("effective_play_rule_version=") || metric.authority.registeredFormula.includes("effective_play_rule_version=effective_play_v2"))
    ? "有效观看：首帧成功展示；视频时长达到 15 秒时实际前台观看至少 15 秒，3 秒以上且不足 15 秒时实际前台观看至少 3 秒，不足 3 秒时须完整播放。仅统计同一有效观看规则下的记录。" : "";
