...
function AuthenticatedProductApp() {
  ...
  return <ProductShell key={state.authScopeKey} contextualNavigation={["/dashboards/public", "/dashboards/mine"].includes(location.pathname)} previewDataSourceEntry={Boolean(developmentDesignPreview(location))} navigationHref={developmentDesignPreview(location) ? previewNavigationHref : undefined}>{routeContent(location, state.session.user)}</ProductShell>;
}
