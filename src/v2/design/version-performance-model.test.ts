...
describe("版本表现演示模型", () => {
  ...
  test("指标名称与公式来自权威投影，M024单独保留存量与冻结规则", () => {
    ...
    expect(versionMetric("M031").definition).toContain("已经完成结果判断");
    expect(versionMetric("M024").attribution).toContain("注册满 2 天");
    expect(versionMetricInputs("M031")).toEqual({ numerator: "成功展示首帧的播放尝试数", denominator: "已完成结果判断的有效播放尝试数" });
  ...
  test("空结果与未成熟不补0，整体活跃次日回访只输出待登记说明", () => {
    ...
    expect(sheets[1].rows.some(row => row.includes("成功展示首帧的播放尝试数"))).toBe(true);
