...
export interface TopicReading { dimension: string; structureId: string; rankingId: string; search: string; descending: boolean; retentionWindow?: 1 | 3 | 7; reactivationWindow?: 48 | 72; scope?: TopicScope }
...
export function parseTopicView(text: string, board?: string, live = false): TopicView | null {
  ...
  try {
    ...
    if (d?.reactivationWindow !== undefined && (board === "5.9" || ![48, 72].includes(d.reactivationWindow))) return null;
    ...
    return { v: 1, active: { start: value.active.start, end: value.active.end }, cohort: { start: value.cohort.start, end: value.cohort.end }, failure: { start: value.failure.start, end: value.failure.end }, compared: value.compared, mixed: value.mixed, ...(d ? { reading: { dimension: d.dimension, structureId: d.structureId, rankingId: d.rankingId, search: d.search, descending: d.descending, ...(d.scope ? { scope: { client: d.scope.client, audience: d.scope.audience } } : {}), ...(d.retentionWindow !== undefined ? { retentionWindow: d.retentionWindow } : {}), ...(d.reactivationWindow !== undefined ? { reactivationWindow: d.reactivationWindow } : {}) } } : {}) };
  } catch { return null; }
}
