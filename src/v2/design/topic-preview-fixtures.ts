...
export const RETENTION_WINDOWS: Record<string, number> = { M020: 1, M021: 3, M022: 7, M023: 30, M024: 1, M100: 2, M025: 3, M040: 1, M041: 3, M042: 7 };
...
function syntheticCohort(date: string, id: string, mixed = false): { base: number; cell: CohortCell } {
  const n = seed(date), watch = WATCH_RETENTION_IDS.includes(id), returning = id === "M024", failed = ["M100", "M025"].includes(id);
  const base = (watch ? 11500 : returning ? 145000 : failed ? 1600 : 5100) + (n % 13) * 137;
  const window = RETENTION_WINDOWS[id], availableAt = shiftDate(date, window);
...
export function topicCard(id: string, range: DateRangeValue, compared: boolean, mixed = false, scope = DEFAULT_TOPIC_SCOPE): DashboardMetricCardModel {
  const metric = topicMetric(id), retention = id in RETENTION_WINDOWS;
  const anchor = REGISTER_IDS.includes(id) ? "注册日期" : WATCH_RETENTION_IDS.includes(id) ? "首次有效观看日期" : ["M100", "M025"].includes(id) ? "首次体验失败日期" : "业务日";
  const label = `${range.start.slice(5)} 至 ${range.end.slice(5)} · ${anchor}${scope !== DEFAULT_TOPIC_SCOPE ? " · " + topicScopeLabel(scope) : ""}`;
  ...
  const read = (date: string) => retention ? syntheticCohort(date, id, mixed).cell.rate : mixed && id === "M101" && date === "2026-09-04" ? null : dailyValue(date, id, scope);
  ...
  const pendingProduction = retention && dates.some(date => ["迟到缓冲中", "待加工"].includes(syntheticCohort(date, id, mixed).cell.status));
  if (!eligible.length) return { metric: identity, result: { status: pendingProduction ? "not_produced" : "immature", label: pendingProduction ? "当前范围尚无已产出结果" : "当前范围暂无观察期已结束的用户", message: pendingProduction ? "观察窗口已结束，部分批次仍在迟到缓冲或等待加工；完成时间待确认。" : `所选 ${anchor} 尚无完整的第 ${RETENTION_WINDOWS[id]} 天结果；最早观察窗口于 ${shiftDate(range.start, RETENTION_WINDOWS[id])} 结束。`, contextLabel: label, retryable: false, validationLabel: "演示数据" } };
  const included = { start: eligible[0], end: eligible.at(-1)! };
  const referenceDates = eligible.map(date => shiftDate(date, -dates.length));
  const summarize = (selected: string[]) => retention ? (() => { const rows = selected.map(date => syntheticCohort(date, id, mixed)).filter(row => row.cell.count !== null); return rows.reduce((s, r) => s + r.cell.count!, 0) / rows.reduce((s, r) => s + r.base, 0); })() : fixturePeriodValue(id, selected, scope);
  ...
  const basis = (selected: string[]): CalculationBasis | undefined => {
    ...
    if (retention) {
      const cohorts=selected.map(date=>syntheticCohort(date,id,mixed)).filter(row=>row.cell.count !== null);
      if (!cohorts.length) return undefined;
      numerator={name:`${RETENTION_WINDOWS[id]===1?"次日":`第${RETENTION_WINDOWS[id]}日`}${WATCH_RETENTION_IDS.includes(id)?"再次有效观看":"留存"}用户数`,value:cohorts.reduce((sum,row)=>sum+row.cell.count!,0),unit:"人"};
  ...
  const points: DashboardMetricTrendPoint[] = dates.map((date, i) => {
    ...
    return { key: `${id}-${date}`, label: date, actualDate: date, calculation:value===null?undefined:basis([date]), value: value === null ? null : { raw: value, display: topicValue(id, value), actualDate: date }, counterpart: compared && previous !== null ? { raw: previous, display: topicValue(id, previous), actualDate: previousDate } : null, differenceDisplay: compared && previous !== null && value !== null ? `${topicValue(id, value - previous, false)} ${unit === "%" ? "个百分点" : unit}` : null, state: value === null ? retention ? "immature" : "not_produced" : "available", stateLabel: value === null ? retention ? syntheticCohort(date, id, mixed).cell.status : "待加工" : "完整" };
  });
...
export function topicDailyCard(id: string, range: DateRangeValue, compared: boolean, mixed = false, scope = DEFAULT_TOPIC_SCOPE): DashboardMetricCardModel {
  let model = topicCard(id, range, compared, mixed, scope);
  if (id in RETENTION_WINDOWS) return periodMetricReading(model, `${rangeLabel(range)} · ${REGISTER_IDS.includes(id) ? "已成熟注册批次" : WATCH_RETENTION_IDS.includes(id) ? "已成熟首次有效观看批次" : ["M100", "M025"].includes(id) ? "已成熟首次体验失败批次" : "已成熟业务日批次"}`);
  const daily = (date: string) => topicCard(id, { start: date, end: date }, false, mixed, scope);
