...
- 构建检查需要同级 `全站埋点与指标体系/` 中的权威指标 Markdown、埋点协作主文档及 HTML 观察版。源文件通过项目既有协作渠道准备，不跳过校验、不将仓库外文档自动纳入本次提交。本地真实日看板与正式生产取数的边界保持不变，生产开放前继续执行本模块的数据准入与部署验收。

### 本地后端启动恢复（2026-09-15）

2026-09-16 PR 提交后本地复核：原前端 5173 与数据库 55432 仍在运行，后端 3000 无监听。沿用已有私有配置重新启动后，后端 health 与前端页面返回 200，匿名会话返回预期 401；未执行账号登录或业务验数。退出原因未确认，未改变账号、Token、数据库或自动恢复机制。仅关闭本次 5183 测试服务，原看板入口保留。
...
| `BI_V2_CORE_OVERVIEW_QUERY_ENABLED` | 核心经营总览首屏批量查询开关，只接受明确 `true` 或 `false`，默认 `false`。只有真实执行器已注入、9 项首屏指标及默认范围均完成数据准入，并达到正式看板首次开放门槛后才能改为 `true`；开启但缺执行器时应用拒绝启动 |
