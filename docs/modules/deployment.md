...
首期内测公网地址确认沿用`https://187.77.129.207.nip.io`；本次只确认域名选择，不代表已完成 DNS、HTTPS、反向代理或服务部署。

首期内部使用保留已确认的看板演示区域，用于查看页面结构与交互；它不是普通生产构建。部署负责人使用`npm run build:internal-preview`生成内测产物，页面必须先登录，并在看板顶部统一说明“真实数据、待验数结果、演示数据”三类来源。每个区块继续显示蓝色“待验数”或橙色“演示数据”，真实查询失败、无记录或未接入时不得回退为演示值。普通`npm run build`仍执行严格演示资源隔离，用于后续不允许演示内容的环境。
...
本次内测发布命令固定为`npm run build:internal-preview`。部署前同时运行`npm run verify:release`和该内测构建；前者验证标准正式构建仍不含演示资源，后者验证内测放行清单与页面级来源提示。服务端真实日看板按环境设置`BI_DAILY_DASHBOARD_QUERY_ENABLED=true`；核心经营总览在九项正式映射、验数与可信水位未完成前保持`BI_V2_CORE_OVERVIEW_QUERY_ENABLED=false`。前端内测标志只控制已确认的展示区，不改变服务端准入、登录、PID权限或真实数据查询结果。
