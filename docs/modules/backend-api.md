...
`GET/POST /api/bi/v2/team/accounts` 列表/创建，`PUT /api/bi/v2/team/accounts/:username` 按 action 调整角色、状态或重置密码。普通账号与未完成首次改密账号返回403；匿名401；写入强制CSRF及生产同源校验。每管理员每分钟最多12次写入，KDF复用现有有界队列。列表20项分页，仅返回公开账号摘要。错误不回显请求体或数据库异常。

沿用 reader/maintainer，已有 analyst兼容但不新增。team入口对自身写入409。团队降级/停用管理员经PostgreSQL事务咨询锁及活动管理员计数保护；CLI保留原维护语义。变更原子更新credential_version并撤销会话；没有明文密码查询或账号删除接口。
