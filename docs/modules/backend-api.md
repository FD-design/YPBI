...
- 所有 `/api/bi/admin/*` 请求统一设置 `Cache-Control: no-store`。生产模式只接受本机 loopback 代理传入的单一 `X-Forwarded-Proto: https` 和单一、有效且与 Fastify 解析结果一致的 `X-Forwarded-For`。协议不安全返回 `426 HTTPS_REQUIRED`；客户端 IP 头缺失、无效或为复合链时返回 `503 MAINTENANCE_PROXY_MISCONFIGURED`，退出接口也不能绕过。Fastify 只信任 `127.0.0.1` / `::1`，因此维护会话和连续失败锁定使用代理链识别出的真实客户端 IP，而不是把所有人都绑定到代理地址。
- 管理 API 的未登录、未启用、登录 Cookie、候选仅验证、保存失败保旧、成功脱敏、退出失效、HTTPS 失败关闭、编码与未知管理路径、普通 404、代理缺失客户端 IP、IPv4/IPv6 可信来源、非可信来源伪造、真实 IP 会话绑定、分客户端锁定和 production 监听契约测试已编写；当前环境缺少 Bun，完整 Bun 套件尚未实际执行。已另用当前 Fastify 运行时执行 14 项代理安全检查并通过。代码与目标代理配置尚未部署，正式开放前仍须在真实 Caddy / Nginx 拓扑复跑黑盒矩阵，未验证前不得把本地通过视为生产安全。
