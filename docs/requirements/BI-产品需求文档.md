...
|---|---|
| 文档 ID | BI-PRD-001 |
| 版本 / 状态 | 1.66 / 核心范围已确认，可按章节与数据准入分模块开发 |
| 优先级 | P0 |
...
| # | 功能点 | 关键细节 |
|---|---|---|
| 1 | V1 维护方式 | 上游暂不支持自动申请或刷新凭据，V1 明确采用人工 Token 轮换；只在首次接入、失效或主动轮换时由授权维护者操作，普通看板、分析和导出不要求用户重复输入 |
...
V1 数据调用流程固定为“维护者人工录入候选 Token → 仅验证或验证并保存 → YPBI 服务端保存有效凭据 → 普通用户发起看板/分析查询 → 服务端按 PID 选择站1或站2并自动携带匹配 Token 与用户名 → 返回经过契约校验的查询结果”。Token 是上游访问凭据，不是业务数据、普通用户身份或 PID 权限来源。
