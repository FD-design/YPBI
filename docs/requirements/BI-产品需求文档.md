...
|---|---|
| 文档 ID | BI-PRD-001 |
| 版本 / 状态 | 2.43 / 临时测试数据会话有效期调整为24小时；正式指标准入独立 |
| 优先级 | P0 |
| 用途 | 产品经理与同事阅读评审、后续 AI 继续澄清并按已确认章节开发和验收的产品需求权威源 |
| 指标权威版本 | 本次需求核对基线为 `v0.37-draft`；包含97项标准指标、9项BI配套分析指标与2项接口诊断返回字段；开发或验收开始时重新读取权威源当前版本，并在实施记录中注明实际使用版本 |
| 更新时间 | 2026-09-23 |
...
2026-09-23补充确认：共享正式 Token 的维护规则不变；同一套生产 YPBI 另提供显式“临时测试数据”模式，用于开发接口联调。用户在维护二次认证后输入测试 Token，验证通过后只为当前普通账号、当前登录会话和当前来源建立 24 小时内存会话；不保存、不替换正式 Token，也不影响其他用户。所有未显式选择测试模式的请求继续使用正式数据。
...
| 12 | 会话隔离 | 测试 Token 只保存在 API 进程内存，使用独立 HttpOnly Cookie 定位；会话同时绑定普通账号安全版本、普通登录 Cookie、可信客户端 IP，24 小时到期，API 重启、退出、改密、停用或角色变化后失效 |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| 2.43 | 2026-09-23 | 临时测试数据会话固定有效24小时；维护二次认证仍为30分钟。账号、普通登录会话、可信来源IP绑定，以及退出、改密、停用、角色变化和API重启失效规则保持不变。 |
