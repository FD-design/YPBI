...
> 本文件是所有修改的第一阅读入口。最后更新：2026-08-26。
...
| 独立产品 BI 平台 | 复用成熟 BI 工作台、12 类图表、下钻、双周期、卡片库和模板管理；首个产品 NewAV，与原 PID/双 Token 数据源完全隔离 | `new-platform/src`、`new-platform/server` | [独立平台架构](./new-platform/architecture.md)、[NewAV API 清单](./new-platform/newav0-api-inventory.md)、[数据与算法审计](./new-platform/data-algorithm-audit.md) | 线上真实数据运行中 | 2026-08-26：完成全模板数据与算法审计 |
...
- NewAV 已确认使用 7 天有效期 Bearer Token；当前测试凭证将于 2026-09-01 22:15:40（UTC+8）到期，届时需替换 `new-platform/.env.local` 并重启 API。金额单位和服务端时区仍需业务复核。
