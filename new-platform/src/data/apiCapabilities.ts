...
const capabilities: Record<AnalysisModelId, ApiAnalysisCapability> = {
  ...
  platform_compare: {
    ...
    limitations: ["当前产品空间仅注册 NewAV；后续产品通过独立产品注册表扩展"]
  },
  content_position: {
  ...
  payment_conversion: {
    ...
    allowedDimensions: ["platform", "date", "channel"],
    allowedMetrics: ["payerCount", "payRate", "revenue", "newRevenue", "currentPaidMembers", "historicalPaidMembers"],
  ...
  acquisition_conversion: {
    ...
    allowedDimensions: ["date", "platform", "channel"],
    allowedMetrics: ["visits", "newUsers", "visitRegisterRate", "dau", "dauUserDays", "viewerUserDays", "adClickCount", "newRevenue", "payerCount"],
    funnelEvents: [],
    filters: ["日期区间", "平台"],
