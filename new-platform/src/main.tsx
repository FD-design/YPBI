...
const platformPid: Record<string, string> = { NewAV: "newav" };
...
function cardScopedFilters(card: DashboardCardConfig, filters: DashboardFilters): DashboardFilters {
  ...
  if (card.platformMode === "single") return { ...filters, mode: "single", platforms: [selected[0] ?? filters.platforms[0] ?? "NewAV"] };
  const fallback = filters.platforms.filter((name) => available.has(name)).slice(0, platformLimit);
  const compared = selected.length >= 2 ? selected.slice(0, platformLimit) : fallback.length >= 2 ? fallback : ["NewAV"];
...
const metricName: Record<MetricId, string> = {
  ...
  pageViews: "广告展示次数",
  pageClicks: "播放错误次数",
  currentPaidMembers: "当前活跃 VIP",
  historicalPaidMembers: "订单总数",
  renewalRate: "续费率"
  ,videoWatchCount: "播放成功次数"
...
function loadTemplatesFromStorage() {
  return normalizeTemplates(loadPersisted<unknown>("newav-bi-templates-v2", dashboardTemplates));
}
...
function loadCardAssetsFromStorage() {
  return normalizeCardAssets(loadPersisted<unknown>("newav-bi-card-assets-v2", initialCardAssets()));
}
...
function makeTreeOption(): EChartsOption {
  return {
    ...
    series: [{
      ...
      data: [
        { name: "推荐流", value: 37.8, children: [{ name: "NewAV 首页焦点位", value: 18.6 }, { name: "NewAV 推荐流", value: 11.4 }, { name: "NewAV 详情页推荐", value: 7.8 }] },
...
function WorkspaceNav({ active }: { active: Workspace }) {
  ...
  return <aside className="workspace-nav">
    <div className="nav-brand"><b>NewAV BI</b><span>独立产品分析工作台</span></div>
...
function GlobalFilterBar({ filters, onChange }: { filters: DashboardFilters; onChange: (filters: DashboardFilters) => void }) {
  ...
  const openPlatformPanel = () => {
    ...
    setPlatformDraft(filters.platforms.length ? filters.platforms : ["NewAV"]);
    setPlatformSearch("");
    setOpenPanel("platform");
  };
  const changeModeDraft = (mode: PlatformMode) => {
    setModeDraft(mode);
    if (mode === "single") setPlatformDraft((current) => [current[0] ?? "NewAV"]);
    if (mode === "compare") setPlatformDraft((current) => [current[0] ?? "NewAV"]);
  };
  ...
  return <>
    <div className="global-filter-bar">
      <button className="filter-trigger platform-trigger" type="button"><span>当前产品</span><b>NewAV</b><small>独立产品数据空间</small></button>
      ...
      <button className="save-view-btn" onClick={() => { localStorage.setItem("newav-bi-global-filters-v2", JSON.stringify(filters)); notify("全局分析范围已保存"); }}>保存范围</button>
    </div>
...
function CardEditDrawer({ card, onChange, onClose }: { card: DashboardCardConfig; onChange: (patch: Partial<DashboardCardConfig>) => void; onClose: () => void }) {
  ...
  return <aside className="card-edit-drawer">
    ...
    <div className="drawer-body">
      ...
      <div className="config-block card-platform-config"><b>产品范围</b><span>当前卡片属于 NewAV 产品空间；后续新增产品时可选择跟随产品或固定产品。</span><div className="platform-mode-tabs compact"><button className="on" onClick={() => onChange({ platformMode: "single", platforms: ["NewAV"] })}>NewAV</button></div></div>
...
function App() {
  ...
  const [globalFilters, setGlobalFilters] = useState<DashboardFilters>(() => loadPersisted("newav-bi-global-filters-v2", { dateRange: "近7日", mode: "single", platforms: ["NewAV"] }));
  const template = templates.find((item) => item.id === templateId) ?? templates[0];
  const systemSurface = systemSurfaceTemplates.find((item) => item.id === templateId) ?? null;
  useEffect(() => {
    try {
      localStorage.setItem("newav-bi-templates-v2", JSON.stringify(templates));
      localStorage.setItem("newav-bi-card-assets-v2", JSON.stringify(cardAssets));
    } catch {
  ...
  return <div className="app">
    <header className="topbar"><div><b>NewAV 产品数据 BI</b><span>真实数据 · 成熟分析工作区</span></div><QueryHealth /></header>
