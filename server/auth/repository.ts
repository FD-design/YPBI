...
export class MemoryAuthRepository implements AuthRepository {
  ...
  async listUsers(search: string, page: number) {
    const matching = [...this.users.values()]
      .filter((user) => `${user.username} ${user.displayName ?? ""}`.toLowerCase().includes(search.toLowerCase()))
      .sort((left, right) => right.createdAt.getTime() - left.createdAt.getTime() || left.username.localeCompare(right.username));
