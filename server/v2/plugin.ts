...
export const v2BiPlugin: FastifyPluginAsync<V2BiPluginOptions> = async (app, options) => {
  ...
  app.get("/catalog/readable-dashboards", async (request, reply) => {
    const principal = await requirePrincipal(request, reply, options.identityProvider, identityTimeoutMs);
    if (!principal) return;
    return dailyDashboardCatalog(Boolean(options.dailyDashboardService));
  });
  ...
  app.post("/queries/dashboards/daily-reading", async (request, reply) => {
    ...
    const principal = await requirePrincipal(request, reply, options.identityProvider, identityTimeoutMs);
    if (!principal) return;
    if (!options.dailyDashboardService) return sendV2Error(request, reply, 422, "DAILY_READING_DISABLED", "当前环境未开启真实看板读取");
    ...
    try {
      const result = dailyDashboardSuccessSchema.safeParse(await options.dailyDashboardService.execute(query.data));
  ...
  app.post("/queries/dashboards/core-overview", async (request, reply) => {
    ...
    try {
      reply.header("cache-control", "no-store");
      const result = await options.coreOverviewQueryService.execute(normalizedQuery, {
