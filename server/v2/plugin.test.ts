...
import { v2BiPlugin, type V2BiPluginOptions, type V2MetricQueryExecutor } from "./plugin";
...
async function createApp(
  ...
  options: {
    ...
    dataEnvironmentResolver?: V2BiPluginOptions["dataEnvironmentResolver"];
  } = {}
) {
  const app = Fastify();
  openApps.push(app);
  await app.register(v2BiPlugin, {
    ...
    dataEnvironmentResolver: options.dataEnvironmentResolver
  });
  return app;
}

describe("v2BiPlugin identity boundary", () => {
  test("未声明数据环境的旧请求保持正式路径且环境状态明确返回 production", async () => {
    const app = await createApp(reader("all"), queryExecutor(), {
      dataEnvironmentResolver: { testAvailable: true, resolveTest: () => ({ status: "missing" }) }
    });

    const response = await app.inject({ method: "GET", url: "/api/bi/v2/data-environment" });

    expect(response.statusCode).toBe(200);
    expect(response.json()).toMatchObject({ success: true, data: { mode: "production", testAvailable: true, expiresAt: null, userName: null } });
  });

  test("显式测试环境缺少有效会话时失败关闭，不会静默回退正式数据", async () => {
    const app = await createApp(reader("all"), queryExecutor(), {
      dataEnvironmentResolver: { testAvailable: true, resolveTest: () => ({ status: "expired" }) }
    });

    const response = await app.inject({ method: "GET", url: "/api/bi/v2/catalog/platforms", headers: { "x-ypbi-data-environment": "test" } });

    expect(response.statusCode).toBe(409);
    expect(response.json().error.code).toBe("DATA_PREVIEW_SESSION_REQUIRED");
  });

  test("非法或未开放的数据环境请求在执行任何查询前拒绝", async () => {
    for (const [header, resolver, code] of [
      ["staging", undefined, "INVALID_DATA_ENVIRONMENT"],
      ["test", { testAvailable: false, resolveTest: () => ({ status: "missing" as const }) }, "DATA_PREVIEW_DISABLED"]
    ] as const) {
      const app = await createApp(reader("all"), queryExecutor(), { dataEnvironmentResolver: resolver });
      const response = await app.inject({ method: "GET", url: "/api/bi/v2/catalog/platforms", headers: { "x-ypbi-data-environment": header } });
      expect(response.statusCode).toBe(header === "staging" ? 400 : 409);
      expect(response.json().error.code).toBe(code);
    }
  });

  test("有效测试会话只在当前查询异步链路注入临时上游配置", async () => {
    const observed: Array<string | undefined> = [];
    const executor = queryExecutor(async (query) => {
      observed.push(currentUpstreamRequestProfile()?.cachePartition);
    ...
    const app = await createApp(maintainer(), executor, {
      dataEnvironmentResolver: {
        ...
        resolveTest: () => ({ status: "active", expiresAt: Date.now() + 60_000, profile: {
          environment: "test", baseUrl: "https://test.example.com", token: "secret-test-token", userName: "test-user", cachePartition: "preview-session-1"
        } })
      }
    });
    const payload = { metricId: "M016", pid: "PH", dateRange: ["2026-09-01", "2026-09-01"], grain: "day" };

    const testResponse = await app.inject({ method: "POST", url: "/api/bi/v2/queries/metrics", headers: { "x-ypbi-data-environment": "test" }, payload });
    const productionResponse = await app.inject({ method: "POST", url: "/api/bi/v2/queries/metrics", payload });

    expect(testResponse.statusCode).toBe(200);
    expect(productionResponse.statusCode).toBe(200);
    expect(observed).toEqual(["preview-session-1", undefined]);
    expect(testResponse.body).not.toContain("secret-test-token");
  });
  ...
  test("新指标定义目录与旧 M016 目录并行且不可缓存", async () => {
    ...
    expect(parsed.data.snapshot.counts).toEqual({ standard: 97, periodDerived: 3, total: 100 });
