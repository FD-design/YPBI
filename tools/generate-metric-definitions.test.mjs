...
test("从权威 Markdown 生成完整且稳定的指标目录", () => {
  ...
  assert.deepEqual(first.snapshot.counts, { standard: 98, periodDerived: 3, total: 101 });
  assert.equal(first.snapshot.authorityVersion, "v0.24-draft");
...
test("界面口径投影从权威指标目录派生业务说明与百分率公式", () => {
  ...
  assert.deepEqual(effectiveViewRate, {
    id: "M036",
    name: "有效观影率",
    definition: "成功起播的播放中，实际观看达到15秒的比例；视频不足15秒且完整播放，也算有效观看\n公式：有效观看次数 ÷ 起播次数 × 100%"
  });
...
test("未接入标准指标与周期派生指标保留真实不可用原因", () => {
  ...
  assert.equal(missingCapability.authority.sourceBuildStatus.code, "capability_missing");
